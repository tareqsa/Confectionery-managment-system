...
public class ShowIncomesPeriod extends JFrame 
{
	...
	public ShowIncomesPeriod() 
	{
		...
		btnNewButton_1 = new JButton("\u05D4\u05E4\u05E7\u05EA \u05D3\u05D5\u05D7");
		btnNewButton_1.addActionListener(new ActionListener() 
		{
			//Choosing incomes date to date for the pdf report  
			public void actionPerformed(ActionEvent arg0) 
			{
				...
				if(!(dateChooser.getDate() == null)  && (!(dateChooser_1.getDate() == null)))
				{
					if (!(dateFromChooser2.before(dateFromChooser1)))
					{
						...
						String incPeriodQuery = "SELECT  `תאריך ושעה`, `שם פרטי`, `שם משפחה`, `מבצע ההזמנה`, `עלות` FROM `orders` WHERE  `תאריך ושעה` BETWEEN '"+sdate+"' AND '"+edate+"' ORDER BY `תאריך ושעה` DESC"; 
					
						try 
						{

							Statement stt = (Statement) Driver.getDatabaseDriver().conn.createStatement();
							ResultSet rs = stt.executeQuery(incPeriodQuery);
							new IncomePeriodReport(rs);
		...
		btnNewButton_1.setBounds(695, 504, 89, 23);
		...
		{
			//Showing incomes from date to date and calculate the sum before and after vat 
					...
					if (!(dateFromChooser2.before(dateFromChooser1)))
					{
						...
						try 
						{
							...
							if(rowsCounter>0)
							{
								...
								lblNewLabel_8.setText(String.valueOf(Math.round(incBeforeVat*100.0)/100.0));
								lblNewLabel_15.setText(String.valueOf(Math.round(((incAfterVat/(1+Vval))*Vval)*100.0)/100.0));
							}
